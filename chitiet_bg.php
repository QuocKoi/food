...
<body>
	...
	$result=mysql_query("SELECT * FROM `burger` WHERE `id_burger`=".$_GET['id']);
	...
	<div class="chitiet container">
		<div id="direction">
			...
			<p>Burger</p>
			<p>></p>
			<p><?=$product['ten_burger']?></p>
		</div>
		<table width="100%" height="600px">
  <tbody>
    <tr>
      <td width="450px">
		<img src="<?=$product['anh_burger']?>">
		</td>
      <td id="inner-chitiet" width="550px">
		<h1><?=$product['ten_burger']?></h1>
		<p>Thực đơn PizzaHut đa dạng và phong phú, có rất nhiều sự lựa chọn cho bạn, gia đình và bạn bè.</p>
		<br>
		  <form  action="cart.php?action=submit" method="post">
			<p>Số Lượng:</p>
			<input type="text" id="quantity" value="1" name="quantity[<?=$product['id_burger']?>]"size="2">
			
			<p>Giá:</p>
			<p id="demo" ><?=$product['gia_burger']?>đ</p>
		  <br>
