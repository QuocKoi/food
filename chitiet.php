...
<body>
	...
	$result=mysql_query("SELECT * FROM `pizza` WHERE `id_pizza`=".$_GET['id']);
	...
	<div class="chitiet container">
		<div id="direction">
			...
			<p>Pizza</p>
			<p>></p>
			<p><?=$product['ten_pizza']?><p>
		</div>
		<table width="100%" height="600px">
  <tbody>
    <tr>
      <td width="450px">
		<img src="<?=$product['anh_pizza']?>">
		</td>
      <td id="inner-chitiet" width="550px">
		<h1><?=$product['ten_pizza']?></h1>
		...
		<br>
			...
			<select id="mySelect" onchange="myFunction()" >
				...
				<option value="Large-39cm" >Large-39cm</option>
			
			</select>
		  <form  action="cart.php?action=submit" method="post">
			<p>Số Lượng</p>
			<input type="text" id="quantity" value="1" name="quantity[<?=$product['id_pizza']?>]"size="2">
			
			<p>Giá:</p>
			<p id="demo" ><?=$product['gia_pizza']?>đ</p>
		  <br>
 ...
 switch(size) {
    ...
    text = "350000";
			  ...
			  document.getElementById("demo").innerHTML = text + "đ";
		}
	</script>
